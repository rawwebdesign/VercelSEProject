...
async function getStoriesByTopic(topic: string): Promise<Story[]> {
  try {
    ...
    const filteredStories = allStories
      .filter((story) => story && story.title && story.title.toLowerCase().includes(topic.toLowerCase()))
  ...
  if (stories.length === 0) {
    return (
      <div className="bg-white p-8 rounded-lg shadow-sm text-center">
        <p className="text-gray-500">No stories found for "{topic}". Try a different search term.</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-8">
      <div className="max-w-4xl">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Stories about "{topic}"</h1>
